...
source.getContentDetails = function(url) {
    ...
    // The VHX API token (window.TOKEN) has a limited lifespan (~24h).
    // If it has expired server-side, the API returns 402/403 even though the session is
    // still valid. Retry once with a freshly-fetched token before requiring re-login.
    if (!sourcesResponse.isOk && (sourcesResponse.code == 402 || sourcesResponse.code == 403)) {
        const freshBearer = getBearer(userAuth);
        sourcesResponse = httpGET(
            `https://api.vhx.tv/videos/${video.id}/files`, 
            true, true, freshBearer
        );
    }

    if (!sourcesResponse.isOk) {
        if (sourcesResponse.code == 402 || sourcesResponse.code == 403)
            throw new LoginRequiredException('Subscription required for premium content');
        throw new ScriptException(`Failed to retrieve video details for video ID ${video.id} [${sourcesResponse.code}]`);
    }
    ...
    for (const source of Object.values(JSON.parse(sourcesResponse.body))) {
